...
#include	"DiskImage/FD/fdd_head_mahalito.h"	/*	追加(Kai2)	*/
...
typedef struct {
	_XDFINFO	xdf;
	UINT32		ptr[VFDD_TRKMAX][VFDD_SECMAX];
	_VFDD_HEAD	head;
...
/*	追加(Kai2)	*/
typedef struct {
	_MHLT_TRK	trk;			/*	トラック情報	*/
	MHLT_SEC	ids;			/*	セクタ情報		*/
	UINT32		*ptr;
} _MHLT_INFTRK, *MHLT_INFTRK;

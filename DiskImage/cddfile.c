...
REG8 sec2048_read(SXSIDEV sxsi, long pos, UINT8 *buf, UINT size) {
	...
	rpos = pos * 2048 + cdinfo->trk[0].start_offset;
	if (file_seeki64(fh, rpos, FSEEK_SET) != pos) {
		return(0xd0);
	}
	while(size) {
		rsize = min(size, 2048);
...
REG8 sec2352_read(SXSIDEV sxsi, long pos, UINT8 *buf, UINT size) {
	...
	while(size) {
		rpos = (pos * 2352) + 16 + cdinfo->trk[0].start_offset;
		if (file_seeki64(fh, rpos, FSEEK_SET) != rpos) {
			return(0xd0);
		}
		rsize = min(size, 2048);
		CPU_REMCLOCK -= rsize;
...
REG8 sec2448_read(SXSIDEV sxsi, long pos, UINT8 *buf, UINT size) {
	...
	fh = cdinfo->fh;
	while(size) {
		rpos = (pos * 2448) + 16 + cdinfo->trk[0].start_offset;
		if (file_seeki64(fh, rpos, FSEEK_SET) != rpos) {
			return(0xd0);
		}
		rsize = min(size, 2048);
	...
	while (size) {
		...
		for (i = 0; i < cdinfo->trks; i++) {
			if (cdinfo->trk[i].str_sec <= pos && pos <= cdinfo->trk[i].end_sec) {
				rpos += (pos - secs) * cdinfo->trk[i].sector_size;
				if (cdinfo->trk[i].sector_size != 2048) {
					rpos += 16;
				}
				break;
			}
			rpos += cdinfo->trk[i].sectors * cdinfo->trk[i].sector_size;
			secs += cdinfo->trk[i].sectors;
		}
		rpos += cdinfo->trk[0].start_offset;
		if (file_seeki64(fh, rpos, FSEEK_SET) != rpos) {
			return(0xd0);
		}
		rsize = min(size, 2048);
